...
static bool                    axp2101_ready = false;

static bool AXP2101_ReadOnce(uint8_t regAddr, uint8_t *data, uint8_t len) {
    if (i2cbus_ == NULL || i2cPMICdev == NULL || data == NULL || len == 0) {
        return false;
    }
    return i2c_master_transmit_receive(i2cPMICdev, &regAddr, 1, data, len, 20) == ESP_OK;
}
...
void Custom_PmicPortInit(I2cMasterBus *i2cbus,uint8_t dev_addr) {
    ...
    axp2101_ready = axp2101.begin(i2cPMICAddress, AXP2101_SLAVE_Read, AXP2101_SLAVE_Write);
    if (axp2101_ready) {
    ...
    ESP_LOGI(TAG, "Battery status: valid=%s connected=%s charging=%s usb=%s percent=%d",
             battery.valid ? "yes" : "no",
...
BatteryStatus ReadBatteryStatus(void) {
    ...
    uint8_t pmic_status[2] = {};
    if (!AXP2101_ReadOnce(XPOWERS_AXP2101_STATUS1, pmic_status, sizeof(pmic_status))) {
        return status;
    }

    status.valid = true;
    status.connected = (pmic_status[0] & (1u << 3)) != 0;
    status.usb = (pmic_status[0] & (1u << 5)) != 0 && (pmic_status[1] & (1u << 3)) == 0;
    status.charging = status.connected && ((pmic_status[1] >> 5) & 0x03u) == 0x01u;
    if (!status.connected) return status;

    uint8_t percent = 0;
    if (AXP2101_ReadOnce(XPOWERS_AXP2101_BAT_PERCENT_DATA, &percent, sizeof(percent)) && percent <= 100) {
